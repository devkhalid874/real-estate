    ...
    @foreach($abouts as $about)
        <tr>
            <td class="align-middle text-center">
                <span class="text-secondary text-xs font-weight-bold">{{ $about->id }}</span>
            </td>
            <td style="white-space: normal; word-wrap: break-word; max-width: 200px;">
                <h6 class="mb-0 text-sm">{{ $about->title }}</h6>
            </td>
            <td style="white-space: normal; word-wrap: break-word; max-width: 300px;">
             <p class="text-xs mb-0">{!! $about->description !!}</p>
            </td>
            <td class="align-middle">
                @if($about->image)
                    <img src="{{ asset('uploads/about/' . $about->image) }}" width="80" class="rounded-2">
                @else
